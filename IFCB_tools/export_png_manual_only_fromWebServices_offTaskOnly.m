...
outputpath = '\\SOSIKNAS1\Lab_data\MVCO\offtask_pngs\'; %USER where to write out pngs
...
if strmatch(urlbase, 'http://ifcb-data.whoi.edu/mvco/'),
    ...
    load class2use_MVCOmanual5
    [ classes_byfile ] = get_annotated_classesMVCO( class2use, manual_list);
    offtask_classes = ~(classes_byfile.classes_checked);
    resultfilelist = classes_byfile.filelist;
else
...
%    category = class2use; %use this syntax to export ALL categories
    %category = {'Chaetoceros', 'Pseudonitzschia' 'Thalassiosira', 'Prorocentrum'}; %use this syntax to export ONLY the listed categories
    ...
    for count2 = 1:length(category);
        ...
        classnum = count2;
        if offtask_classes(filecount,classnum),
