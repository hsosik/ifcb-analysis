...
in_url = '/Volumes/TOSHIBA EXT/whoi_data/D2012/'; %USER web services to access data
...
%hdrfiles = cellstr([repmat(in_url,length(filelist),1) filelist repmat('.hdr', length(filelist), 1)]);%  To use with web services

hdrfiles = cellstr([repmat(in_url,length(filelist),1) filelist(:,1:9) repmat('/',length(filelist),1)  filelist repmat('.hdr', length(filelist), 1)]);
