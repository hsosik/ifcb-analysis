...
classpath_generic = '\\queenrose\test\classTB\xxxx\'; %USER class file location, leave xxxx in place of 4 digit year
in_url = 'http://toast.tamu.edu/ifcb7_new_data/'; %USER web services to access data
yrrange = 2012:2012; %USER
...
hdrfiles = cellstr([repmat(in_url,length(filelist),1) filelist repmat('.hdr', length(filelist), 1)]);
...
clear mdate filelist class2use classcount classcount_above_optthresh classcount_above_adhocthresh filecount yr* classfiles in_url classpath_generic num2dostr
