...
for classnum = 1:length(class_ciliate),
    ml_analyzed_struct.(char(class_ciliate(classnum))) = NaN(1,length(ml_analyzed));
end;
    ...
    for filecount = 1:length(filelist),
        ...
        for classnum = 1:length(class_cat),
            if class_cat(classnum) <= numclass1,
                ...
                biovol(mode_ind(filecount)).(char(class2use_here(class_cat(classnum)))) = [targets.Biovolume(cind)*micron_factor.^3];
                eqdiam(mode_ind(filecount)).(char(class2use_here(class_cat(classnum)))) = [targets.EquivDiameter(cind)*micron_factor];
            else %subdivide case
               if exist('class2use_sub4', 'var'),
