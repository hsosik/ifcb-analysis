function biovolume_summary_allCSV(datasetStr,year_range)
...
switch datasetStr
    ...
    case 'Oleander'
        ...
        classpath_generic = '\\vast\proj\omics\sosik\ensemble\output\oleander\SnapshotEnsembleClassifier_EfficientNet_n30_ep20.dyn\';
...
%    classpath = [classpath_generic filesep ystr filesep];
    classpath = [classpath_generic filesep num2str(yr) filesep];
   ...
   % pathall = strcat(feapath_generic, names(:,1:5), filesep, names(:,1:9), filesep);
    if ~mvco_flag
        feafiles = strcat(pathall2, regexprep(names, '.csv', '_fea_v4.csv'));
...
%            feafiles(ii) = cellstr( strcat(feapath_base, names_temp(:,1:24), xall(ii,:)));
             feafiles(ii) = cellstr( strcat(feapath_generic, ystr, filesep, names_temp(:,1:9),filesep, char(regexprep(names(ii),'.csv', '_fea_v4.csv'))));
        end
        ii = strmatch('I', names);
        names_temp = char(names{ii});
        if ~isempty(ii)
            %feafiles(ii) = cellstr( strcat(feapath_base, names_temp(:,1:21), xall(ii,:)));      
            feafiles(ii) = cellstr( strcat(feapath_generic, ystr, filesep, names_temp(:,1:14),filesep, char(regexprep(names(ii),'.csv', '_fea_v4.csv'))));
        end
        clear temp* pathall classpath xall 
    end
    ...
    filelist = regexprep(cellstr(names), '.csv', '');
    clear names
    ...
    %mdate = datenum(meta_data.sample_time, 'inputformat', 'yyyy-mm-dd HH:MM:ss+00:00');
    ...
    %classTable = load_class_scores_csv(classfiles{1});
    classTable = readtable(classfiles{1}, 'VariableNamingRule', 'preserve');

%    class2use = deblank(classTable.class_labels);
    class2use = classTable.Properties.VariableNames(2:end)';
...
%                 [groupcount(filecount,:), groupbiovol(filecount,:), groupC(filecount,:), groupcount_above_optthresh(filecount,:), groupbiovol_above_optthresh(filecount,:), groupC_above_optthresh(filecount,:), groupcount_above_adhocthresh(filecount,:), groupbiovol_above_adhocthresh(filecount,:), groupC_above_adhocthresh(filecount,:), group2use, groupFeaList(filecount,:), groupPidList(filecount,:)] = summarize_biovol_classGroup_h5(classfiles{filecount}, feafiles{filecount}, adhocthresh, optthresh_group, pidlist_flag, group_tab);
                [groupcount(filecount,:), groupbiovol(filecount,:), groupC(filecount,:), groupcount_above_optthresh(filecount,:), groupbiovol_above_optthresh(filecount,:), groupC_above_optthresh(filecount,:), groupcount_above_adhocthresh(filecount,:), groupbiovol_above_adhocthresh(filecount,:), groupC_above_adhocthresh(filecount,:), group2use, groupFeaList(filecount,:), groupPidList(filecount,:)] = summarize_biovol_classGroup_csv_hierarchy(classfiles{filecount}, feafiles{filecount}, adhocthresh, optthresh_group, pidlist_flag, group_tab, groupXgroup);
            else
                [classcount(filecount,:), classbiovol(filecount,:), classC(filecount,:), classcount_above_optthresh(filecount,:), classbiovol_above_optthresh(filecount,:), classC_above_optthresh(filecount,:), classcount_above_adhocthresh(filecount,:), classbiovol_above_adhocthresh(filecount,:), classC_above_adhocthresh(filecount,:), class2use, classFeaList(filecount,:), classPidList] = summarize_biovol_class_csv(classfiles{filecount}, feafiles{filecount}, adhocthresh, optthresh, pidlist_flag);
                % group data:
%                 [groupcount(filecount,:), groupbiovol(filecount,:), groupC(filecount,:), groupcount_above_optthresh(filecount,:), groupbiovol_above_optthresh(filecount,:), groupC_above_optthresh(filecount,:), groupcount_above_adhocthresh(filecount,:), groupbiovol_above_adhocthresh(filecount,:), groupC_above_adhocthresh(filecount,:), group2use, groupFeaList(filecount,:), groupPidList] = summarize_biovol_classGroup_h5(classfiles{filecount}, feafiles{filecount}, adhocthresh, optthresh_group, pidlist_flag, group_tab);
                [groupcount(filecount,:), groupbiovol(filecount,:), groupC(filecount,:), groupcount_above_optthresh(filecount,:), groupbiovol_above_optthresh(filecount,:), groupC_above_optthresh(filecount,:), groupcount_above_adhocthresh(filecount,:), groupbiovol_above_adhocthresh(filecount,:), groupC_above_adhocthresh(filecount,:), group2use, groupFeaList(filecount,:), groupPidList] = summarize_biovol_classGroup_csv_hierarchy(classfiles{filecount}, feafiles{filecount}, adhocthresh, optthresh_group, pidlist_flag, group_tab, groupXgroup);
            end
    ...
    save([resultpath 'summary_biovol_allCSV_min20_' num2str(yr)] , 'class2use', 'classcount*', 'classbiovol*', 'classC*', 'meta_data', 'mdate', 'filelist', 'classpath_generic', 'feapath_generic', 'adhocthresh', 'classXstat', 'optthresh', ...
        'group2use', 'groupcount*', 'groupbiovol*', 'groupC*', 'meta_data', 'mdate', 'filelist', 'groupXstat', 'optthresh_group', 'val_stats')
    save([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'lists'] , 'class2use', 'filelist', 'classpath_generic', 'feapath_generic', 'classFeaList*', '-v7.3')
    save([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'lists_group'] , 'group2use', 'filelist', 'classpath_generic', 'feapath_generic', 'groupFeaList*', '-v7.3') 
    disp('results saved: ')
    disp([resultpath 'summary_biovol_allCSV_min20_' num2str(yr)])
    disp([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'lists'])
    disp([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'lists_group'])
    if pidlist_flag 
        save([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'list_pid'] , 'class2use', 'filelist', 'classpath_generic', 'feapath_generic', 'classPidList', '-v7.3')
        disp([resultpath 'summary_biovol_allCSV_min20_' num2str(yr) 'lists_pid'])
    end
