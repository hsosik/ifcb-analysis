function [ mdate_mat, y_mat, yearlist, yd ] = timeseries2ydmat_median( mdate, y )
...
%in y) and output a matrix of median values for each yearday by year 
...
for count = 1:length(yearlist),    
    ...
    for day = yd(1):yd(end),
        ...
        if ~isempty(ii),
            y_mat(day,count) = nanmedian(y(iii(ii)),1);
        end;
    end;
end;
