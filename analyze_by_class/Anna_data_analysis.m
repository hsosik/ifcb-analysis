...
for classcount=1:length(classes),
    ...
    [Ciliate_week_mat.(classes{classcount}), Ciliate_weekstd.(classes{classcount}), mdate_wkmat, yd_wk] =ydmat2weeklymat(Ciliate_mat.(classes{classcount}), yearlist);
end

 for classcount=1:length(classes); %plots week bins of all ciliate data
     figure
     h=plot(yd_wk, Ciliate_week_mat.(classes{classcount}), '.-');
     legend(num2str((2006:2013)'), 'location', 'south');
     title(classes{classcount}, 'fontsize', 10, 'fontname', 'arial');
