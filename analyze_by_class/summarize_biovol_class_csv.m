...
classTable = readtable(classfile, 'VariableNamingRule', 'preserve');
class_labels = classTable.Properties.VariableNames(2:end);
...
classTable_roi_numbers = str2num(char(cellfun(@(x) x(end-4:end),classTable.pid,'un',0)));
[~,ia,ib] = intersect(feastruct.data(:,1), classTable_roi_numbers);
%classTable.scores_feamatch(ia,:) = classTable.scores(ib,:);
classTable.scores_feamatch(ia,:) = classTable{ib,2:end};
