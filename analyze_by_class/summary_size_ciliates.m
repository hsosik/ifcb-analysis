...
for daycount = 1:length(unqday),
    ...
    for classcount = 1:length(classes),
        ...
        iii = find(eqdiam_day(daycount).(classes{classcount}) >= 40);
        if ~isempty(iii),
...
eqdiam10_20ciliate = sum(eqdiam10_20_mat(:,ind_ciliate(4)),2);
eqdiam20_30ciliate = sum(eqdiam20_30_mat(:,ind_ciliate(4)),2);
eqdiam30_40ciliate = sum(eqdiam30_40_mat(:,ind_ciliate(4)),2);
eqdiam40_infciliate = sum(eqdiam40_inf_mat(:,ind_ciliate(4)),2);
