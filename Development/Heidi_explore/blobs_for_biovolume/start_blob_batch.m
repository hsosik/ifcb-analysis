...
%daydir = dir([in_dir_base 'D2024020*']);
...
for ii = 1:length(daydir)
    ...
    bins_temp = dir([in_dir_temp '*.roi']);
        if ~isempty(bins_temp)

        bins_temp = regexprep({bins_temp.name}', '.roi', '');
      ...
      %   daystr = char(bins_temp(1)); daystr = daystr(1:14);
        ...
        bins_done = regexprep({bins_done.name}', '_blobs_v4.zip', '');
...
if nfiles > 0
    n = 1
    batch_blobs(in_dir(n:end), out_dir_blob(n:end), bins(n:end), parallel_proc_flag);
end
