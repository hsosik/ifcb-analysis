...
function [ ] = batch_features_test(in_dir, filelist, out_dir, in_dir_blob, parflag )
...
if not(debug),
    ...
    if ~isempty(in_dir_blob)    
        parfor filecount = 1:length(filelist)
             bin_features_test(in_dir, [char(filelist(filecount)) '.zip'], out_dir, [], in_dir_blob{filecount});
        end
    else
        parfor filecount = 1:length(filelist)
           bin_features_test(in_dir, [char(filelist(filecount)) '.zip'], out_dir, []);
        end;
    end
else
