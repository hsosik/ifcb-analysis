function [ ] = bin_features_test( in_dir, file, out_dir, opt1 , in_dir_blob, log_callback)
...
if nt > 0,
    temp.pid = targets_blob.pid;
    [ feature_mat, featitles, multiblob_features, multiblob_titles ] = make_feature_matrices_test(temp);

    %write the compiled feature csv file
    fileout = regexprep(file, '.zip', '_fea_v3test.csv');
    fileout = regexprep(fileout, '.roi', '_fea_v3test.csv');
    log(['SAVING ' fileout]);
   
    csvwrite_with_headers( [out_dir fileout], feature_mat, featitles );
    
    %write the raw multi-blob features to separate csv file
    fileout = regexprep(file, '.zip', '_multiblob_v3test.csv');   
    fileout = regexprep(fileout, '.roi', '_multiblob_v3test.csv');   
