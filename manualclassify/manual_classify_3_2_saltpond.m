...
pick_mode = 'correct_or_subdivide';
...
resultpath = 'C:\work\IFCB010\ManualClassify\'; %USER set
% resultpath = '/mellon/projects/2012/ifcb_saltpond/manualclassify/'; %USER set
%resultpath = '\\mellon\saltpond\autoclassify_14May2012\'; %USER set
classpath = '\\mellon\saltpond\autoclassify_14May2012\'; %USER set
basedir = '\\mellon\saltpond\D2012\';
stitchpath = '/mellon/data/ifcb/saltpond/stitches';  %%USER set, roi stitch info files
class_filestr = ''; %USER set, string appended on roi name for class files

% filespec = 'IFCB0_2012_075*'; %USER set; include at least year and day; time optional
...
filespec = 'D20120315*'; %USER set; include at least year and day; time optional
year = filespec(2:5);

%% Define basedir to review a specific day, otherwise review most recent day
%basedir = [basedir_all{1} 'D2012/D20120424/'];
%if ~exist('basedir'),
%    dlist = dir([basedir_all{1} 'D2012/D*']);
%    latestdir = dlist(end).name;
%    basedir = [basedir_all{1} 'D2012/' latestdir '/']; 
%end

%streampath = [basedir]; % needed for batch case
