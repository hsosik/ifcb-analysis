...
temp = load([resultpath 'manual_list']); %load the manual list detailing annotate mode for each sample file
manual_list = temp.manual_list; clear temp
...
while filecount <= length(filelist),
    ...
    %OFFTASKMANUAL heidi
    %special case for viewing only off task classes from MVCO manual_list
    [~, temp] = fileparts(filelist{filecount});
    iii = strmatch(temp, classes_byfile.filelist);
    class2view = find(classes_byfile.classes_checked(iii,:) == 0);
    %OFFTASKMANUAL heidi
    classcount = 1;
