...
batch_classnum = [48:52]; %USER which class do you want to view in batch mode, Heidi 10/7/09, only works for correct_or_subdivide for now
...
resultpath = 'X:\IFCB1\ifcb_data_mvco_jun06\Manual_fromClass\'; %USER set
classpath = 'D:\work\IFCB1\ifcb_data_mvco_jun06\class2010_24may07\'; %USER set
basedir = 'Z:\';  %%USER set, roi files, adc files
streampath = [basedir 'IFCB1_2010_???\']; %USER set, which year / day to analyze
stitchpath = 'D:\work\IFCB1\ifcb_data_mvco_jun06\stitch2010';  %%USER set, roi stitch info files
class_filestr = '_class_24May07'; %USER set, string appended on roi name for class files

%filelist = dir([resultpath 'IFCB1_2009_???_00*']);
filelist = get_filelist_manual([resultpath 'manual_list'],3,[2010]); %manual_list, column to use, year to find
