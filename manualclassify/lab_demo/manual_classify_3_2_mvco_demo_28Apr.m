...
filenum2start = 2;  %USER select file number to begin (within the chose day)
pick_mode = 'raw_roi'; %USER choose one from case list below
...
%filespec = 'IFCB5_2012_028*'; %USER set; include at least year and day; time optional
filespec = 'IFCB1_2012_119*'; %USER set; include at least year and day; time optional
...
basedir =  '\\queenrose\g_work_ifcb1\Demo_28Apr2012\ifcb_data_MVCO_jun06\';
...
for filecount = filenum2start:length(filelist),
    ...
    minproctime = .073;  %use for dates after 13 Oct 08
    ...
    looktime = adcdata(end,9)- adcdata(end,1)*minproctime - sum(adcdata(longproc_ind+1,8)-adcdata(longproc_ind,9)) + length(longproc_ind)*minproctime;       
