close all
resultpath = '\\queenrose\g_work_ifcb1\Demo_27Apr2013\ManualClassify\';
...
[class2use_plot, ind] = setdiff(class2use, {'other'});
temp = char(filelist); temp = cellstr(temp(:,1:end-4));
t = load('\\RASPBERRY\d_work\IFCB1\code_mar10_mvco\ml_analyzed_all');
[~,~,ii] = intersect(temp,t.filelist_all);
ml = t.ml_analyzed(ii);  clear t temp
for ii = 1:length(filelist),
    ...
    bar(classcount_plot(sind,ii)./ml(ii),.5);
