...
load '/Volumes/IFCB_data/IFCB014_OkeanosExplorerAug2013/metadata/metadata.mat'
...
load '/Users/markmiller/Documents/Experiments/OKEX_Cruise/summary_old/alt_summary_old/count_manual_05Feb2014.mat'
matdate_alt=matdate;
ml_analyzed_alt=ml_analyzed;

tintinnid_alt=classcount(:,90);
[matdate_bin, classcount_bin, ml_analyzed_bin] = make_hour_bins(matdate_alt,tintinnid_alt, ml_analyzed_alt);

tintinnid_perml_alt=classcount_bin./ml_analyzed_bin;

min_diff= 1/86400;
match_ind_alt=NaN(size(matdate_bin));


for ii=1:length(matdate_bin);
    [min_ii, i2]=min(abs(matdate_bin(ii)-Date_full));
...
load '/Volumes/IFCB_products/IFCB010_OkeanosExplorerAug2013/Manual_fromClass/summary/count_manual_08May2014.mat'

tintinnid_norm=classcount(:,90);
[matdate_bin_norm, classcount_bin_norm, ml_analyzed_bin_norm] = make_hour_bins(matdate,tintinnid_norm, ml_analyzed);
tintinnid_perml_norm=classcount_bin_norm./ml_analyzed_bin_norm;

match_ind_norm=NaN(size(matdate_bin_norm));
min_diff= 2/1440;


for ii=1:length(matdate_bin_norm);
    [min_ii, i2]=min(abs(matdate_bin_norm(ii)-Date_full));
