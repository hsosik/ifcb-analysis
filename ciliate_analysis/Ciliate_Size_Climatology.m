...
%C=
figure %plotting normalized sizes averaged over all years for each month (overlayed)
plot(diambins, nanmean(log(meanh_norm),3))
xlabel('diameter (\mum)', 'fontsize', 14)
ylabel('Cells (mL^{-1} \mum{-1})', 'fontsize', 14)
%legend(num2str((1:12)'));
%xlim([0 50]);
legend('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec', 'fontsize', 12);
set(gca,'fontsize',14, 'YTick',[0 0.1 0.2 0.3])
...
for year_count=1:length(year_list);
    ...
    set(gcf, 'position', [225 65 940 610])
end
