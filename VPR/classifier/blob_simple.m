...
img_blob = imclose(img_blob, seD);
img_blob = imdilate(img_blob, seD);
img_blob = bwmorph(img_blob, 'thin', 2); %20 oct 2011, Heidi thinks 3 times here might be better than previous 1
...
if config.plot, %go to graphing routine if requested
    ...
    %img_proc_plot(target, blob_s, img_blob , img_dark)
    pause
end;
